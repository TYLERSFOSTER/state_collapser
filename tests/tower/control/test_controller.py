...
def test_controller_does_not_descend_toward_non_executable_unclosed_tier() -> None:
    controller = ActiveTierController()
    signal = TierSignalState(visit_count=3, td_error_ema=0.0, success_count=3)
    lower_signal = TierSignalState()
    signals_by_tier = {
...
def test_controller_still_descends_toward_executable_unclosed_tier() -> None:
    controller = ActiveTierController()
    signal = TierSignalState(visit_count=3, td_error_ema=0.0, success_count=3)
    lower_signal = TierSignalState()
    signals_by_tier = {
...
def test_controller_lifts_toward_nearest_unclosed_executable_tier() -> None:
    controller = ActiveTierController()
    signal = TierSignalState(visit_count=3, td_error_ema=0.0, success_count=3)
    middle_signal = TierSignalState()
    upstairs_signal = TierSignalState()
    signals_by_tier = {
        0: upstairs_signal,
        1: middle_signal,
        2: signal,
    ...
    configs = {
        0: TierControlConfig(epsilon=0.0, min_visit_count=2, success_threshold=0.5),
        1: TierControlConfig(epsilon=0.0, min_visit_count=2, success_threshold=0.5),
        2: TierControlConfig(epsilon=0.0, min_visit_count=2, success_threshold=0.5),
    }

    decision = controller.decide(
        ActiveTierState(active_tier=2, tier_state=None, deepest_known_tier=2),
        signal,
        configs[2],
        signals_by_tier=signals_by_tier,
        tier_configs=configs,
        frozen_context=_context(),
        training_due=False,
        tier_is_executable=lambda tier: tier == 0 or tier == 2,
    )
