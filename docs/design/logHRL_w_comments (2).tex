...
This intuition, that putting a hierarchical structure on a computing problem can induce a logarithmic speed-up, is also hidden in the CNN architecture of U-Nets \cite{FB15} \textcolor{red}{this idea is only 11 years old, was SOTA at that time, and now it's completely classical. Mind blowing pace of movement in this field!}{\color{blue} [Yeah! Totally agree!]} and ResNets \cite{HZRS15}, where it again induces a logarithmic improvement. 
...
For residual correction, it is {\em multigrid}-like, in the sense of \cite{AhamedZakariaeiHaberEliasofMGE2026}. MgNet \textcolor{red}{Do you mean this \cite{he2019mgnet} article?} {\color{blue} Ok yeah I'm a little lost... Does the 2026 paper use a framework developed in the 2019 paper? My understanding is the both introduce a kind of gradient scaling between successive tiers of the U-Net, and uses these scaling factors to say mroe concrete things about the computational efficiency of the U-Net. Let's investigate this more carefully.} \textcolor{red}{I think the 2019 paper uses different scales on the grid by using a simple, piecewise linear function on each piece of the grid but then simply stride it through -- simple convolution (Lemma 1 of their paper) thrown in with FEM, but I don't think they scale the gradient, but there is no hierarchy of resolutions. The 2026 paper uses Monte Carlo methods to scale the gradient, and use the UNet idea for their hierarchy of resolutions. I think maybe replace `MgNet' with `MGE', but keep the original reference?} makes this correspondence especially explicit: restriction maps pass information to a coarser grid, smoothing operations remove local high-frequency error, and prolongation maps lift coarse corrections back to the fine scale. Read through this lens, a residual CNN is not merely a stack of feature extractors; it is also an iterative correction scheme in which coarse representations control the search for fine-scale residual structure.
...
Rewards are action-local after any necessary state augmentation, and quotient rewards are trained or estimated as direct-image rewards (sum and max?). \textcolor{red}{Sum? as per Eq (3) of \cite{HutsebautBuysseHRLSurvey}. The max is probably taken over all quotients, as in Eq (4) of \cite{HutsebautBuysseHRLSurvey}. Is that even practical?. For both equations, I imagine there is a derivation using Statistics.} {\color{blue} I think the right think to do here is clarify that the specific computational meaning of ``direct image'' is variable here. Basically any reasonable theory of aggregation works, with sum, weighted sum, RMS, and max being reasonable examples.}\textcolor{red}{Well.. when you had the pushforward for the quotienting $q:S \longrightarrow \bar{S}$ written out as $q_*F - = \Sigma_{s\in -} F(s)$, my immediate thought was that you want to use sum because it fits nicely with the logarithmic limit. Would that be possible if, instead of a sum, a max was used? I imagine that might work for pullbacks instead of pushforwards, and you do need pushforwards because you want to aggregate across the fibers instead of ignoring them. Another related point: the correspondence with GNNs says that one should use sums or max or RMS because these are permutation invariant, but of course weighted sum isn't, unless the weights are permuted, too, and that is something that will need to be kept track of in the pushforward.} 
...
When the tower is approximately balanced, the product is of the same order as $\#\{\text{discovered states}\}$. \textcolor{red}{A small explanation on what `balanced' means loosely would be helpful here. Perhaps by referring to numbers in the cumulative reward kernel modulo transition properties}
\end{assumptions}
...
{\color{blue} Aaaah ok I think my poor informal wording in this Theorem and its setup has created a lot of confusion for you later in the paper about the path space. ...I think it's kind of obscured what it is I'm up to with the path space, so let me explain it differently here.

First, you should be thinking of $\bold{Path}(H)$ as ``something {\em waaaaay} worse/more complicated than the fundamental group $\pi_{1}(H,\ast)$ (for some choice of node $\ast$ in $H$). The fundamental group $\pi_{1}(H,\ast)$ is hard to understand in general. It is ``loops at $\ast$ modulo homotopy,'' i.e., $\bold{Loop}(H, \ast)/\sim$. Loops at $\ast$ are just paths that have start and end point $\ast$. So the space $\bold{Path}(H)$ is way bigger than the insane set that we quotient in order to get the fundamental group. All of this is to say that the path space $\bold{Path}(H)$ is sort of unknowable. The {\em path space} is the set of all paths {\em of all lengths} in $H$. The {\em length-0} paths are the same things as nodes. The {\em length-1} paths are just edges. All paths of length $\ge 2$ contain more than one edge. 

The relevance to RL is that a ``point'' $\gamma\in\bold{Path}(H)$ amounts to a history in $H$, i.e., a sequence of consecutive nodes (states) connected by edges (primitive actions). So the reason for even thinking about $\bold{Path}(H)$ in the context of RL is that $\bold{Path}(H)$ is the moduli space of things that RL training needs to search over and optimize.

It is {\em very much} like the path formulation of QM, where reality is somehow optimizing over all paths to minimize this physical quantity $S$ (``action'' in a different sense than we're using here). In RL, it's not this physical quantity $S$ we're trying to minimize. Rather, we're trying to maximize a quantity called ``total reward,'' i.e., total payout if we take a given history through state space.

{\em The real problem} we run into is that because RL is trying to optimize over this moduli space of histories $\bold{Path}(H)$, we have to actually do some kind of geometry on $\bold{Path}(H)$. We can't just treat it like a set. The path formulation of QM runs into this exact problem. In QM, it is formulated as ``the problem of summing over all Feynman diagrams.'' It's really an attempt to take an integral, roughly of the form
$$
\underset{\text{Path}(TX)}{\int}\!\!\!\!\!\!L(\gamma)\,d\gamma,
$$
where $\text{Path}(TX)$ denotes all histories through phase space, and where ``$L(\gamma)$'' is some physical quantity you can compute, given a particular history $\gamma$. 
...
Kontsevich's big achievement around ``{\em stable maps} and ``{\em virtual fundamental classes} was to show that using algebro-geometric tools, you can build something {\em like} $\text{Path}(TX)$, but much more controlled, namely a moduli space of stable curves, where integration makes sense, in the form of an intersection theory. I am conflating some thigns here, and being loose with ideas, but it's the correct general picture. \cite{COGP} \cite{Katz} \cite{Kontsevich95}

So one of the big moves in the paper below is scraping {\em just enough geometry} out of $\bold{Path}(H)$, in the form of this growing ``volume power series'' $\mathcal{P}-Vol$ to show that... let me say it this way: If you think about binary search / HNSW a lot like some CS people do, you get this strong sense that ``logarithmic addressing speeds up computation because it compresses the volume of a space in a logarithmic way.'' The thing that is less obvious is that a logarithmic compression also happens to this more terrifying space $\bold{Path}(H)$, because to say that correctly, we would need some notion of volume in $\bold{Path}(H)$. Even when $H$ is finite, $\bold{Path}(H)$ isn't, so this need for volume in $\bold{Path}(H)$ is real. It is very much a version of needing an integral like $\underset{\text{Path}(TX)}{\int}\!\!\!\!\!\!L(\gamma)\,d\gamma$.  The trick of using $P-Vol$ is that it pushing the non-fininteness of the integral into a literal series.

...Maybe we should figure out how to fold some of that above into this text itself. Is it good motivation for what we do?
}\textcolor{red}{I think that is very nicely put! But I don't see how the `balanced' picture comes in.}
...
\end{quote}
In this paper, such a policy will be represented by a stochastic kernel $\pi$ that assigns probabilities to admissible next actions, either from the current state alone or from whatever history/observation the learner is allowed to use. When no ambiguity is important, we write this in the Markov form $\pi(-|s)$. When the dependence on rollout history matters, we allow $\pi$ to be evaluated on a history or on an observation extracted from that history.\textcolor{red}{Hope this is not a naive question: by abstract nonsense, \textbf{Path}($H$) is a collection of edges with well-defined source and target maps. Should we allow a history to be viewed as a single action in the past? I imagine we want to conserve the fact that we are working with a Markov Property?} {\color{blue} No this is a good question! I would word it a little differently: a point in $\bold{Path}(H)$ encodes a sequence of actions in $H$ with source/target conditions: $\text{src}(\alpha_{i+1})=\text{tgt}(\alpha_{i})$. So I would say ``\textbf{Path}($H$) is a collection of {\em collections of} edges with well-defined, {\em matching-up} source and target maps.'' Regardless of all that, you're making a really good point: The case where the policy $\pi$ depends on whole histories is a problem for \texttt{state\_collapser} because \texttt{state\_collapser} doesn't yet have that feature, and the reason it's not there is because we need to stay in the Markovian regime for the results in this paper. Figuring out how to push past them is a subject for a future project. So we should probably drop that last sentence, maybe replace it with an explanation about our restricted perspective here.} \textcolor{red}{Got it.} 
\end{ermark}
...
\end{enumerate}
\end{definition}
\textcolor{red}{Very interesting! Is there a nice way to compute the weights? Like some sort of Parseval's identity?} {\color{blue} Oh that's interesting! I never thought about that possibility. The convention I had in mind is a naive one that just fixes an initial decay parameter $0<\omega<1$, and then defines $w_{i}:=\omega^{i}$. That convention comes directly from \cite{SBRL}. It's like the most naive thing you could do. ...Yeah it would be interesting to do an FFT during training to try to optimize the decay weights. I have seen things where an FFT is used at the bottom-most stage of a U-Net, in order to optimize a filter applied on the frequency at that stage.} \textcolor{red}{I see. That is, by design, diminishing the contributions of the distant states, assuming that the states are all distinct! If not, then the contribution of states that kinds wrap back on themselves at a certain point is also diminished. An FFT probably bypasses this, and would be an extension to $k$th order Markov Regime}
\begin{remark}
\normalfont
...
\end{ermark}
\textcolor{red}{Sorry, what does the $\Delta$ stand for? I have never taken a course on Markov Processes, so I don't know if that's standard terminology. Sorry!. Scratch that. It's a distribution} {\color{blue} No no. You were right to catch that. You'll find this interesting: It's has the same meaning you're familiar with. It's an $(N-1)$-simplex, where $N:=\#\,\bold{Path}_{k}(H)$ !!! Really is is the space of probability distributions on the set $\bold{Path}_{k}(H)$, but that's just
$$
\Big\{(c_{\gamma})_{\gamma\in\bold{Path}_{k}(H)}:\sum_{\gamma}0\le c_{\gamma}\le 1\;\text{and}\;c_{\gamma}=1\Big\},
$$
which is the defintion of a geometric simplex (as opposed to a combinatorial simplex).}
\textcolor{red}{Oh, yes! That just reminded me of Tae-Dane Bradley's thesis\cite{bradley2020interface}. First time I heard her on a Sean Carroll's podcast where she explained this -- my mind was blown! I also remember I brought this up to you and you told me your PhD thesis was concerned with this. I told myself I'd read your thesis after that. That just reminds me that I need to!}
\begin{ermark}
