...
Both {\em binary search} \cite{CLRS3} and {\em hierarchical navigable small worlds} ({\em HNSW}) \cite{JK} \cite{FGKLL} \cite{MY16} provide logarithmic speed-up to search algorithms by implementing geometric addressing.
 ...
 For binary search, {\color{blue} [...the point is to state what the addressing is like, and what the resulting log speed-up for search is, and like how it comes from the addressing.,, \ Tyler]}

For HNSW, {\color{blue} [...the point is to state what the addressing is like, and what the resulting log speed-up for search is, and like how it comes from the addressing.,, \ Tyler]}

We can also cast the logarithmic speed-up in terms of path specification. {\color{blue} [...the idea here is that traversal distances are being contracted according fo an explicitly logarithmic scheme... \ Tyler]}
    \vskip -.5cm
\begin{figure}[!ht]
    $$
...
Abdul Malik applied these ideas to graph ML learning in \cite{MalikThesis} {\color{blue} Summarize Abdul's use of graph tower for simplex detection... \textemdash\ Tyer}
\end{example}
...
For residual correction, it is {\em multigrid}-like, in the sense of \cite{AhamedZakariaeiHaberEliasofMGE2026} {\color{blue} [...MgNet makes the numerical-analysis correspondence even more explicit: CNN pooling corresponds to multigrid restriction, and CNN feature extraction corresponds to multigrid smoothing... \textemdash\ Tyler]}
...
- [Hiearchical structure in momowhatever counterpoint texture is kind of ideal]
...
- [Picture is a tower of graphs with massive contractions from one tier down to next.]
...
[They have to be stated in a heuristic way here, because we don't want to introduce the full machinery...]
...
{\bf Three curtains.}
\normalfont
{\color{blue} [Just need to explain the real world / environment model dichotmy that drove a lot of the thinking in this project and a lot of the design work... This is important because the ``3 curtains of reality]}
\end{ermark}
...
{\color{blue} [...connecting blurb... \textemdash\ Tyler]}
...
\normalfont
{\color{blue} [This is basically just more ``quick alignement with \cite{SBRL},'' except that I want to make sure to state clearly here that the policy is a stochastic kernel, i.e., conditional probabiliity $\pi:\mathcal{S}_{H}\longrightarrow\mathcal{P}(\mathcal{A}_{H})$ such that each $\pi(-|s)$ is supported on $\mathcal{A}_{H}(s)$. This is a weird way to say it, and kind of hints at ``sheaf on tangent space,'' but it is important for this paper because it hints at the loosening of the last condition, and this is exactly what happens in implementation, in skipping across cosets... can view it as a uniformizing across cossets.... \textemdash\ Tyler]}

{\color{blue} [Does it make any sense to try to say single sentence about microlocal feel that mathematicians would know how to follow? \textemdash\ Tyler]}
...
\end{quote}
{\color{blue} [...Explain the policy $\pi$, and that we will assume that it is ... see immediately below... \textemdash\ Tyler]}
\end{ermark}
...
{\color{blue} [Oh the other thing I wanted to say here is that because the full path space also adds states, a cumulative reward kernel/function actually has this extra data of rewards for just being stationary. We could interpret these as rewards for loop transitions or like ``stay this step''... This is important, because there is this running thing we need to address that engineers like E will get endlessly confused about: ``What about loops? What about cycles? What about loops here? What about cycles here? What about loops? What about cycles? \textemdash\ Tyler]}
\end{remark}
...
\normalfont
A policy \(\pi_\theta\) assigns to each history, or to a chosen observation of that history, a probability distribution over admissible next actions {\color{blue} [... There's a basic problem here... I need to go back up in the text and just kin dof hint at policy on histories not just states... but also ...it just needs to be integratedcorrectly \textemdash\ Tyler]}.  Together with the environment transition mechanism and reward computation, it induces a probability distribution
\[
...
\begin{subsection}{Formalisms for hierarchical RL}
 - {\color{blue} [This is where to cite a ton of the literature... Honestly I'm pretty daunted by this seciton... \textemdash\ Tyler]}
 
 \cite{SPS}
 \cite{Diet}
  \cite{SBRL}
 \cite{PSTQ}
 \cite{HutsebautBuysseHRLSurvey},

\begin{remark}
{\bf Subtask / supertask / component task / abstract task.}
\normalfont

{\color{blue}[Computing, like CS and engineering, is {\em sub-structure} focussed, and not very {\em quotient-structure} focussed. \textemdash\ Tyler]}

Insight from \cite{MalikThesis}: in lots of graph-structured problems, we can can improve search times by putting the graph into a tower of randomly chosen contractions that functions as a {|em graph quotient HNSW} query system.

- [Super-tasks as underspecified task outlines...]
...
$$
{\color{blue} [The danger here is in making this check quadratic in $k$, which could happen if we do checks of all outgoing edges at the ends of all edges. At the same time, it's dynamic information... \textemdash\ Tyler]} 

{\color{blue} [...There's also like a giant message pass you can do on the ambient graph... there are just lots of parallel things we cna do here...]}
\end{remark}
...
\normalfont 
{\color{blue} [Describe the picture immediately below... \textemdash\ Tyler]}
\begin{figure}[!h]
	\vskip -.2cm
    $$
    \!\!\!\!
...
\[
\Pi_i^{\mathcal S}=\{C_{i,\alpha}^{\mathcal S}\}_{\alpha\in I_i^{\mathcal S}}
\quad\text{of}\quad\mathcal{S}_t^0,
\]
and the \emph{action-side partition table} is a finite labeled partition
\[
\Pi_i^{\mathcal A}=\{C_{i,\beta}^{\mathcal A}\}_{\beta\in I_i^{\mathcal A}}
\quad\text{of}\quad\mathcal{A}_t^0
\]
such that each cell $C_{i,\beta}^{\mathcal A}$ lies over a single state partition cell $C_{i,\alpha}^{\mathcal S}$, in the sense that there exists a state partition cell $C_{i,\alpha}^{\mathcal S}$ in $\Pi_i^{\mathcal S}$ such that for all actions $\alpha\in C_{i,\beta}^{\mathcal A}$, we have
	$$
	\text{src}(\alpha)\in C_{i,\beta}^{\mathcal S}
	$$
{\color{blue} [Notational conflict in indices $\alpha$... \textemdash\ Tyler]} This ensures that at tier-$i$, we have a tangent bundle-like map 
$$
...
$$
\Pi_i^{\mathcal S}\longrightarrow\Pi_0^{\mathcal A}
$$
{\color{blue} [Do we need to say more about why this exists? \textemdash\ Tyler]}

{\color{blue} [Describe inductive procedure that gives next tier... \textemdash\ Tyler]}
...
{\color{blue} [Explain why the data structures in memory that represent the contracted graphs are really just fibered Young tableux \cite{YoungTab}, where the decorations are state or action names. \textemdash\ Tyler]}

Finally, the tower is nested because these partitions coarsen with $i$. Each state-cell at tier $i$ lies inside a unique state-cell at tier $i+1$, and likewise each action-cell at tier $i$ lies inside a unique action-cell at tier $i+1$. Thus the actual tower object is best understood as:
\begin{enumerate}
    \item the fixed discovered base graph $G_t^0$,
    \item a nested tower of labeled state partitions $\Pi_0^{\mathcal S},\Pi_1^{\mathcal S},\dots$,
    \item a nested tower of labeled action partitions $\Pi_0^{\mathcal A},\Pi_1^{\mathcal A},\dots$,
    \item and, at each tier, the outgoing-incidence pointers from state-cells to action-cells.
\end{enumerate}
...
\subsection{Runtime analysis of Algorithm~\ref{alg: successive_quotients_nested_cosets}}
