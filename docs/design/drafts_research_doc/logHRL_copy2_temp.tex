...
{\color{blue} [Where was I going with this?]}
\end{example}
...
{\color{blue} [The set up has to be stated in a heuristic way here, because we don't want to introduce the full machinery...]}
...
{\color{blue} [Blurb to ease into what we're about to actually do..]}
...
{\color{blue} [Need connecting blurb here]}
...
{\color{blue} [The following is no good, and needs to be turned into an actually fucking "what people have done before and how it's different than what we're diong" section: The relevant background is the options framework of Sutton, Precup, and Singh \cite{SPS}, Dietterich's MAXQ decomposition \cite{Diet}, the hierarchical RL overview in Sutton and Barto \cite{SBRL}, and the more recent survey literature such as \cite{PSTQ} and \cite{HutsebautBuysseHRLSurvey}. These works establish the standard HRL expectation that hierarchy should make long-horizon control easier by replacing flat primitive action search with search over temporally or structurally extended abstractions.]}
...
\begin{figure}[!h]
    ...
    $$
    \vskip -.5cm
    \caption{{\color{blue} [...Need blurb here to explain picture. The blurb should help the reader see how abstract cosets in a contracted graph are meaningful.]}}
\end{figure}
...
\normalfont 
{\color{blue} [Your blurb below is wrong. Look at the image in the assocaited pdf. You essentially have teh picture upside down please look before describing Also fix figure caption below]}

{\color{green} Figure~\ref{fig: dynamic tower} should be read from bottom to top. At the bottom is the discovered base graph $G_t^0$, containing the concrete states and primitive actions the agent has exposed by time $t$. Each higher level $G_t^i$ is obtained by contracting one block of the base-tier contraction schema and recording the resulting state/action cosets. The vertical maps are quotient maps: they send concrete states and actions to their current abstract cells, while preserving enough outgoing-action information for an agent to move at the abstract level.}
\begin{figure}[!h]
	\vskip -.2cm
    $$
    \!\!\!\!
    ...
    \vskip -.5cm
    \caption{{\color{blue} [...]}}
    {\color{green} Suggested caption: Dynamic quotient tower over the discovered graph. Each tier contracts an additional block of base-tier actions, producing coarser state/action cosets while retaining outgoing-action information needed for abstract movement.}
...
{\color{blue} [The construction isn't described correctly  below, I mean just in words. It sounds static when it's really this recursive construction... Please show me rewrite of this part that describes actual construction.]}
...
{\color{green} Yes: it exists because the action-side partition is defined by outgoing incidence. Every state-cell $C\in\Pi_i^{\mathcal S}$ is represented by a collection of base states, and each of those base states already points to outgoing edge buckets in the base graph. Coarsening the state partition coarsens these outgoing buckets in parallel, so the merged state-cell inherits a well-defined outgoing action-cell collection $\mathrm{Out}_i(C)$.}

{\color{blue} [Describe inductive procedure that gives next tier... \textemdash\ Tyler]}

{\color{green} The inductive step from tier $i-1$ to tier $i$ is driven by the $i$th block $\Sigma^i$ of the base-edge contraction schema. For each edge $a\xrightarrow{\ e\ }b$ in this block, let $C_a$ and $C_b$ be the current state-cells containing its endpoints. If $C_a\neq C_b$, we create a new tier-$i$ state-cell $C_{ab}$ by coalescing $C_a$ and $C_b$, and we create the corresponding tier-$i$ action-cell data by coalescing their outgoing action-cell collections. Pointers from all nodes in $C_a\cup C_b$ are then initiated to $C_{ab}$, and $C_{ab}$ is pointed to its merged outgoing-action collection. Repeating this for all arrows in $\Sigma^i$ gives the next partition layer.}
...
\normalfont
{\color{blue} [What's really important here is the weird way you jump across arrows that aren't actually having your node as source....]}
\end{remark}
