...
% Math packages (Duplicates removed)
...
% Graphics & Color (Duplicates removed)
\usepackage{graphicx}
\usepackage{xcolor}
...
% --- ADDED FOR MULTIPLE AUTHORS WITH AFFILIATIONS ---
\usepackage{authblk} 

% Hyperlinks (Duplicates removed, placed at the end of preamble)
\usepackage{hyperref}
...
\author[1]{Tyler Foster}
\author[2]{Abdullah N. Malik}
\affil[1]{Unaffiliated}
\affil[2]{Tallahassee State College, Tallahassee, FL, USA}
\date{\today}
...
The original RL/counterpoint problem, the path-space geometry motivation, the quotient-task viewpoint, the dynamic tower construction, the nested state/action partition data structure, the coarsening/Young diagram interpretation, and the decisions about which mathematical claims this paper is trying to make are Tyler Foster and Abdullah Malik's work. The design documents and engineer-continuity reports in the \texttt{state\_collapser} and \texttt{HGraphML} repositories consistently record Foster and Malik as the source of the substantive mathematical direction and of the critical corrections that shaped this research document and its accompanying repositories.
...
\ \ \ \ \ All figures were created by Tyler Foster in \texttt{draw.io} and \texttt{LaTeX}, with one instance of GPT-image assistance: Figure \ref{} includes a $3$-armed, red tubular surface. That surface is a GPT-image enhanced of a line drawing that Foster did.
\end{minipage}
\end{center}
\newpage
...
This intuition, that putting a hierarchical structure on a computing problem can induce a logarithmic speed-up, is also hidden in the CNN architecture of U-Nets \cite{FB15} and ResNets \cite{HZRS15}, where it again induces a logarithmic improvement. 

Indeed, the speed-up provided by HNSW comes from an geometrically thinned hierarchy of coarsened spaces. The original HNSW paper \cite{JK}, Kleinberg describes the hierarchy as links separated by characteristic distance scale, enabling logarithmic complexity scaling. A U-Net does essentially the same thing: each downsampling layer reduces the number of sites and increases the physical scale represented by one feature. Thus, a U-Net is HNSW-like for spatial/frequency information flow.
\begin{figure}[!h]
	\vskip -.5cm
    $$
    \!\!\!\!\!\!\!\!
...
\begin{figure}[!h]
    ...
    $$
    \vskip -.5cm
    \caption{Making a choice among ``the next $5$ possibilities'' $7$ consecutive times leads to $78\,125$ possible sequences of choices.}
...
{\bf\em Rewards are local.} Rewards are action-local after any necessary state augmentation, and quotient rewards are trained or estimated as an aggregate over all pre-image actions, be it via \texttt{sum}, \texttt{avg}, or power means like \texttt{rms} or other $L^{p}$-norms, like the $L^{1}$-norm \texttt{sum}$\circ$\texttt{abs} and the $L^\infty$-norm \texttt{max}.
 

{\bf\em State collapse is geometric.} There are positive integers $b_m,b_{m-1},\ldots,b_0$ and nonnegative residual terms $\epsilon_i$ such that the tower controller can choose the coarsest address and each subsequent refinement with expected cost
$O(\log b_i)+\epsilon_i$, with induced address capacity $\prod_{i=0}^{m} b_i \ge \#\{\text{discovered states}\}$.

{\color{blue} The real issue here is to clarify when/how this $O(\log b_i)+\epsilon_i$ expectation occurs in nature. It is basically a "sparse and connected" assumption, but repackaged...}

%When the tower is approximately balanced, the product is of the same order as $\#\{\text{discovered states}\}$.

%A tower address system is approximately balanced on a finite set $X$ if its
%address fibers have uniformly controlled skew. Equivalently, if
%$q_i:X_{i-1}\to X_i$ are the tierwise address maps and
%$$
%b_i := \max_{x_i\in X_i}|q_i^{-1}(x_i)|,
%$$
%then the address capacity
%$$
%|X_N|\prod_{i=1}^N b_i
%$$
%is comparable to the actual set size $|X|=|X_0|$:
%$$
%|X_N|\prod_{i=1}^N b_i \asymp |X_0|.
%$$
\end{assumptions}

\begin{theorem}\label{theorem: main}
When Assumptions \ref{ass: simple ass} hold, the tower controller's search cost $\cost_{\mathcal{T}}(\Omega)$ has a logarithmic upperbound:
$$
    \cost_{\mathcal{T}}(\Omega)
    \quad\le\quad
    C\sum_{i=0}^{m}\log b_i \;+\; \sum_{i=0}^{m}\epsilon_i
$$
for a model-dependent constant $C$. This provides a logarithmic improvement to flat search over $\Omega$, which has enumeration cost $
...
\textcolor{red}{How about an analogy with the Kleene Star? It's similar to the operator $\bold{Path}(-)$ (we are talking about discrete paths as opposed to continuous paths, after all). We want to tease out a regular language from the Kleene Starred set, and the training process for an RL is basically allowing it to crawl over some unknown weighted FSM. I think `balanced' here would mean the absence of an agent entirely, unlike an MDP.}
...
\begin{definition}
\normalfont
A {\em finite directed  state/action-graph} $H$ \textcolor{red} is a finite directed graph
...
{\bf Region of 3-dimensional space as grid lattice:}
\normalfont
...
\begin{remark}
{\bf Decompositions of path space.}
\normalfont
The path space of $H$ is the discrete analogue of the usual topological path space $\Path(X):=C^0([0,1],X)$ associated to a topological space $X$
...
\begin{ermark}
\label{remark:my-naive-question}
...
The agent is rewarded and/or penalized for its actions as it travels through state space. {\em Solving} a given RL problem means coming up with some kind of policy that controls the agent's movement through state/action-space, such that \textcolor{red}{text missing}
\end{quote}
...
{\bf }
\normalfont
We use the language of stochastic kernels, ie..\textcolor{red}{i.e.,}, conditional probabilities, to describe the rewards an agent receives as it traverses an RL environment. All the key ideas in this paper are already at play in the special case that all reward sampling processes are deterministic, so that reward distributions and reward kernels become reward values and reward functions, respectively.
...
\end{remark}
 ...
 \normalfont
By a {\em reward distribution} \textcolor{red}{r} associated to a given action $\alpha\in\mathcal{A}_{H}$, we mean a probability distribution $P_{\!\alpha}$ on the real number line $\mathbb{R}$ and some process that samples from it:
$$
r\;\sim\;P_{\!\alpha}
$$
\end{definition}
...
\normalfont
When every one of these distributions $P_{\!\alpha}$ is a Dirac delta supported at some point $r(\alpha)\in\mathbb{R}$, we can understand the action-local stochastic kernel as a deterministic {\em action-local reward function}
$
...
\item
There exists some sequence $w_{\bullet}=(w_{0}, \;w_{1},\;w_{2},\;\cdots,\;w_{k-1})$ of {\em decay weights} $w_{i}\in\mathbb{R}_{\ge0}$, such that for any history $\gamma=(s_{0}\xrightarrow{\ \ \alpha_{1}\ }s_{1}\xrightarrow{\ \ \alpha_{2}\ }\cdots\xrightarrow{\ \alpha_{k-1}\ }s_{k-1}\xrightarrow{\ \alpha_{k}\ }s_{k})$, of any finite length $k$, its associated reward can be computed as the convolution product $r(\gamma)=w_{\bullet}\ast r(\alpha_{\bullet})$, i.e.,
$$
...
$
r:\bold{Path}(H)\longrightarrow\mathbb{R}
$,
...
\begin{remark}
\normalfont
Note that because $\mathcal{A}_{H}\subset\bold{Path}(H)$, we can interpret a cummulative \textcolor{red}{cumulative} reward kernel as some sort of free ``weighted convolutive extension of an action-local reward kernel to a reward kernel on full histories.''
...
\begin{ermark}
{\bf Policies as models-plus-weights on a concrete machine}
\normalfont
...
The only part of this whole situation that’s {\em really} different from vanilla ML is that for RL, our labelled training data is ``stored'' in a {\em virtual rollout database}, and that our strategy for pulling entries from this database is different than for vanilla ML.
...
$$
Likewise, we define the associated {\em viewed subgraph at time $t$}, denoted $G^{0}_{t}$, to be the subgraph of $H$ formed by the union of the visited subgraph $H^{0}_{t}$ at time step $t$ and all outgoing $1$-hop neighborhoods $N^{\text{out}\!}_{1}(s)$ in $H$ at nodes in $H^{0}_{t}$ at time step $t$:
$$
G^{0}_{t}
\;:=\;
...
\!\!N^{\text{out}\!}_{1}(s)
$$
We let $\mathcal{S}^{0}_{t}$ \textcolor{red}{I am a little confused. We are defining $G^0_t$ by using $\mathcal{S}^0_t$ -- the set of vertices of the graph $H$ -- to begin with. Did you mean to write $\mathcal{S}^0_{G,t}$ or something? Or maybe a comment addressing a recycling notation would be helpful} {\color{blue} Yeah you're right. I like your proposed ``$\mathcal{S}^{0}_{G,t}$.'' Technically it's not a problem, but it's confusing in current notation.}\textcolor{red}{Yeah, just a remark that we're recycling notation here would all that's needed. Your original notation is simpler, but it has the risk of being confused with the existing set of states.} and $\mathcal{A}^{0}_{t}$ denote the states and actions in $G^{0}_{t}$, i.e., the nodes and edges of $G^{0}_{t}$, respectively.
\end{definition}
\textcolor{red}{I think a comment on why this is different from the temporal-based subtask in the reviews is warranted?} {\color{blue} I agree we should add this comment here. We need to be careful here. We need to make sure to be direct about any extent to which temporal-based subtask is categorically dual to our picture. Another important thing here is that there is no learning in our tower. We don't isolate meaningful subtasks... we try instead to create a uniformly collapsed HNSW-type thing. It's important that this is supposed to give a learning speed-up independent of anything specific thing being learned. {\em We are not claiming that any model is able to learn something that then makes the tower effective. We're claiming, with proof, that the hardcoded table building procedure gives speed-up for classical CS reasons, not because we can learn something about it.} That's very different than a paper like \url{https://arxiv.org/pdf/2602.10015}. Are there any papers that show an understanding of the intrinsic/classical source of the speed-up that everyone is discovering in hierarchical learning. Like one thing kind of bold we're saying in this paper is "HRL speed-up is really just $n$-ary search in disguise, which suggest it has very little to do with intrinsic subtasks.''}\textcolor{red}{I think the flow charts accomplish that plenty! They should be linked somewhere.. appendix? I can make that up. Also, I don't know, but the hierarchial papers you linked are definitely not about creating hierarchy in our sense. The vertices are ranked or some flags are added based on a ranking criterion or some other add on, but there is no compression happening. Not that I can see! I think I'll put the papers up on NoteLM and see if that's happening. All ideas trace back to \cite{geisberger2008contraction}, and there's a diploma thesis with the same name as the workshop paper that has more details\url{https://ae.iti.kit.edu/download/diploma_thesis_geisberger.pdf}. Curiously, there is a the $\oplus$ and $\otimes$ log semiring operation that somehow shows up in one of the papers.. }
\begin{remark}
\normalfont
...
\normalfont
Given a history as in \eqref{eq: gamma history}, we decorate each node $s$ of $G^{0}_{t}$, at each time step $t$, with its set $\mathcal{A}_{H}(s)$ of outgoing edges  {\em inside} our ambient, hidden state/action-graph $H$.
\end{definition}
...
We implement the quotient-tower in RAM by maintaining a nested system of labeled partitions of the base state set and base edge set under successive coarsening operations. This places the method near the classical partition-based data-structural viewpoint in algorithms \cite{PaigeTarjan1987} and \cite{HabibPaulViennot1999}. That said, the construction we now describe is not a {\em partition refinement algorithm} in the usual sense. Because the operative updates here encode graph edge contractions, they take the form of {\em coarsenings} of coupled state and action coset partitions. The tower is best understood as a coupled\textcolor{red}{,} nested partition system adapted to contraction-driven quotient formation.
...
$$
Note that each of the cells of $\Pi_0^{\mathcal A}$ is naturally associated to exactly one of the cells of $\Pi_0^{\mathcal S}$, namely its source state \textcolor{red}{I understand that even if there are there sink states in $H$, it makes no difference to the existence of the map below, since $\mathcal{S}^0_t$ is a viewed subgraph at time $t$. However, if our stutter convention is to let $s_j \longrightarrow s_j$ be, then $\left\{s_j\right\}\subset$Out$(s_j)$. I hope it doesn't break anything in the second map below} {\color{blue} You're right to catch this. We need to think through this.}. In this way, me have a natural map
$$
...
We apply the graph quotienting procedure to this data structure as a sequence of successive parallel coarsening of two coupled partition systems. Assume, by finite induction, that at tier $i$ the \emph{state-side partition table} is a finite\textcolor{red}{,} labeled partition
\[
\Pi_i^{\mathcal S}=\{C_{i,\lambda}^{\mathcal S}\}_{\lambda\in I_i^{\mathcal S}}
\quad\text{of}\quad\mathcal{S}_t^0,
\]
and the \emph{action-side partition table} is a finite\textcolor{red}{,} labeled partition
\[
...
The tower is nested because these partitions coarsen with $i$. Each state-cell at tier $i$ lies \textcolor{red}{I don't know if there's an English word that captures $\subseteq$. The word `lies', I think, conveys $\subset$.} {\color{blue} Ha. good point... ``is or is in'' ha ha?} inside a unique state-cell at tier $i+1$, and likewise each action-cell at tier $i$ lies inside a unique action-cell at tier $i+1$. Thus the actual tower object is best understood as: (i) the fixed discovered base graph $G_t^0$, (ii) a nested tower of labeled state partitions $\Pi_0^{\mathcal S},\Pi_1^{\mathcal S},\dots$, (iii) a nested tower of labeled action partitions $\Pi_0^{\mathcal A},\Pi_1^{\mathcal A},\dots$, (iv) and, at each tier, the outgoing-incidence pointers from state-cells to action-cells.

The pseudocode below should be read in these terms.\textcolor{red}{In the \texttt{require}.. does the state collection $\mathcal{S}^0_t$ at $t$ not come by default with the graph? Also, what happens if the initial explicity or implicit partion (the schema) isn't given? Should an iid have that done for us? Worth repeating here at the outset IMO. Sorry for making you scroll and not writing down the comments inside the algorithm, and rather leave them here}

\textcolor{red}{Shouldn't the function only need the graph itself? If you'd like to keep it, I think in line 1 of the algo, you want $\mathcal{S}^0_t$, not $s_t$ as the input. In Line 19.. is stutter convention very uniform across the field? If yes, I suppose we don't need to explain the word `chosen' in this line. In Line 26, should the loop not be over $\Pi^{\mathcal{S}}_{i-1}$ instead of $\mathcal{S}_t^0$?}
{\color{blue} I need to think hard about this...}
...
%    \Statex
    ...
    \State $i \gets 1$
    \While{$i \le d$} \label{line:main-while-loop-algo-init}
        ...
        \For{$s\in\mathcal{S}_t^0$} \label{line:last-loop-algo-init}
...
{\color{violet} \textbf{PO-directed Codex note:} This is also the conceptual location of the degenerate-tier control issue. If $\mathrm{Out}_i([s]_i)=\text{\O}$, then the tier-$i$ state-cell may still be a legitimate quotient cell, but it is not an executable decision locus for the current agent. The runtime should therefore lift/refine toward a finer tier until it reaches a state-cell with nonempty outgoing-action data, and only treat the situation as genuinely trivial if tier $0$ is also empty. This matters because it is the operational version of the liftability hypothesis: quotient addresses help only when they can be routed back to executable behavior, rather than handed to a learner as empty action surfaces.}
\end{remark}
...
\end{equation}
...There's also a natural argument that the expect expected nature of the sparsity in state/action graphs gives us the following expected value of $m_{t}$ is
$$
m_{t}
\;\sim\;
n_{t}\,d
...
$
Note every base-tier edge is processed exactly once by the contraction schedule.
...
First, the algorithm copies the previous partition tables forward and later re-initiates the node pointers for all states. The explicit final loop in Line \ref{line:last-loop-algo-init} already contributes $O(n_t)$ work per tier, regardless of how many contractions actually occur in $\Sigma_0^i$.
...
\begin{itemize}
    ...
    \item If cell coalescence is implemented by a standard union-by-size or union-by-rank discipline\textcolor{red}{I think a reference here would be nice to have} {\color{blue} I agree. I'll try to find one.}, then within a fixed tier\textcolor{red}{,} each base element changes its canonical representative only logarithmically many times. In that case, one obtains the tier\textcolor{red}{-}wise bounds
    \[
    ...
    \]
\end{itemize}
\textcolor{red}{Would one coalescences not be treated as a single step in almost any language? Then $U^{\mathcal{S}}_i = \left\vert\Pi^{\mathcal{S}}_i\right\vert$ and $U^{\mathcal{A}}_i = \left\vert\Pi^{\mathcal{A}}_i\right\vert$, and the $U_i$'s would depend on $i$}. {\color{blue} Ok this is good. I'm going to address this and a comment below at once. So the first important thing to realize is that I am very purposefully trying to draw an analogy between the dimension $d_\text{dim}$ of a state space and the expected degree $d_\text{deg}$ in a graph. This is because in a positively-directed grid lattice, we have $d_\text{deg}=d_\text{dim}$. In a bi-directed grid lattice, we have $d_\text{deg}=2\,d_\text{dim}$. The strategy for building the intrinsically meaningful tower for a grid lattice goes "contract $x$, then contract $y$, then contract $z$. As a partition across the whole state space, this looks like $\Sigma^x \sqcup \Sigma^y \sqcup \Sigma^z$. So the general pattern really is that I'm going to treat the number of cells in this partition as something like ``the last vestige of dimension I have for a general graph.''

The other thing that's going on is that I'm then treating this value $d$ as if it were a logarithm: $d\sim \text{log}_{\ell}\,n_{t}$. This is again coming from the grid lattice case, so we should be really careful about what we're saying here. The idea is that in a grid lattice {\em in a compact region of $3$-space}, if each axis has $\ell$ grid points, then the total number of points in the grid lattice region is $\ell^{d}$. Because we're imagining points in our graph to be states in state space, this is the identity $n_{t}=\ell^{d}$, which gives us an expected value $d\sim \text{log}_{\ell}n_{t}$.
}\textcolor{red}{Got it! Perfect, then}
...
\textcolor{red}{I would maybe use a breakable algorithm} {\color{blue} Can you explain?}\textcolor{red}{Like longtable, you can have the algorithm break over two pages. For Algorithm 2, I basically removed the floating \texttt{algorithm} and figure environment. But.. the caption wasn't what you had, so I then also had to introduce another environment called breakablealgorithm just to put a line below and above the algorithm. Now the algorithm breaks over two pages. I didn't make any changes to algorithm 1.}
...
$$
An entropy-based variant replaces support size by $\exp(H(\mu_{\theta,k}))$.\textcolor{red}{What is $H$? I thought it referred to a graph. I don't know what the exponential means here with that distribution.} {\color{blue} Yeah this is confusing. The idea is that what's ``really'' happening in RL is that we're selecting from path space with a certain distribution. As an agent learns a policy, it stops seeing all paths/histories equally, It's starts to have strong preferences for what states it even going to explore in the first place. }
\end{definition}
...
\sum_{k\ge0}d^k
$
is useful as a slogan for path-space explosion, but it is not a finite complexity invariant unless it is truncated or discounted. All statements below use a finite horizon $K$ or an explicitly discounted variant.
\end{warning}
...
The reward definitions above already allow reward distributions or reward kernels. For the theorem, the relevant operation is the following direct-image construction.\textcolor{red}{:}

{\color{blue}\textsf{[PO-requested Codex comment, 2026-05-31.] Foster asked that future engineers keep a separate documentation issue visible here: the BBB counterpoint first serious learning evaluation did \emph{not} fail because of direct-image reward aggregation; its pathology was full/near-full first-projection collapse plus lift/action-realization. However, direct-image aggregation will matter in later tower benchmarks. If a quotient tier uses a reward/value threshold for admissibility, lifting, pruning, training, or controller selection, then the aggregation rule can make lower/refinement-tier thresholds bind upper/coarser-tier performance. A max aggregator turns such a threshold into an existence gate, ``there is a good refinement in this fiber,'' while a mean/expectation aggregator turns it into a typicality gate, ``the average refinement is good enough.'' Either choice changes what an upper-tier policy is allowed to regard as promising, and can impose effective ceilings or feasibility constraints on the full system. This should be documented as a reward-interface design issue before quotient-tier performance is interpreted as learner performance.}}

\begin{definition}[{\bf Direct-image reward on an action cell}]\label{def:direct-image-action-cell-reward}
\normalfont
Let $D\in\Pi_i^{\mathcal A}$ be a tier-$i$ action-cell, and let $r:\mathcal{A}_t^0\to\R$ be an action-local reward function. The direct-image reward of $D$ is
$$
r_i(D)
:=
\E\bigl[r(\alpha)\mid \alpha\in D\bigr],
$$
...
\normalfont
Rewards are action-local after any necessary state augmentation, and quotient rewards are trained or estimated as direct-image rewards on action cells, as in Proposition~\ref{prop:reward-descent-partition}.
\end{assumption}
...
$
and nonnegative residual terms $\epsilon_i$ such that \textcolor{red}{I think having a reference for the justification for $U^{\mathcal{S}}_i$ and $U^{\mathcal{A}}_i$ would be helpful in convincing the logarithmic growth. I am not debating the existence of the numbers, though. The other three assumptions are reasonable, but I'm having a hard time wrapping my head around the logarithm. And understanding this is probably crucial to the next phase of the project.} {\color{blue} I agree. I need to go through and make appropriate changes.} the tower controller can choose the coarsest address and each subsequent refinement with expected cost
$$
O(\log b_i)+\epsilon_i,
$$
...
\begin{section}{Merge status and unresolved editorial junctions}

\begin{subsection}{Source chronology used for this merged working copy}

This merged working copy uses
\path{docs/design/logHRL_w_comments.tex}
as its spine.  The reason is chronological and structural.  The older
\path{docs/design/drafts_research_doc/logHRL_copy*_temp.tex}
files are May 25 snapshots.  The standalone mathematical-model drafts in
\path{docs/design/drafts_research_doc/}
are also May 25 sources, and the rewrite bundle in that directory is an
earlier formalization pass.  The cleaner
\path{docs/design/logHRL.tex}
file is newer than those drafts, dated May 29, but the commented
\path{docs/design/logHRL_w_comments.tex}
file is dated May 31 and contains the later PM/PO comment stream and the
expanded attribution language.

The supplemental note
\path{docs/design/logHRL_state_vs_path_address_capacity_note.tex}
is dated May 29 and should be read as a targeted correction note for the
state-address/path-volume notation issue.  It is not an independent competing
version of the paper.  The log/tropical and adic notes are later research
notes.  They are mathematically related to the paper, but they are not simply
alternate versions of the main text.  They should be folded into the main
paper only by an explicit editorial decision, most naturally as a future
appendix, sequel section, or separate companion note.

\end{subsection}

\begin{subsection}{Places where the most recent state is clear}

The main-text lineage is clear: the current working state is the May 31
commented draft.  In particular, the current draft already supersedes the
May 25 draft copies, the earlier mathematical-model document, and the May 29
clean \texttt{logHRL} file wherever they overlap.  The current draft also
contains the later Abdul Malik attribution, PM comment stream, degenerate-tier
control note, and the more explicit partition-table description of the tower
in RAM.  Those should be preserved unless deliberately revised.

The explicit quotient-tower algorithm should be read against the newer
partition-table/Young-diagram runtime picture, not against the older
``equivalence relation hull'' or informal message-passing descriptions.  Where
older drafts describe quotient formation at a higher level, the current
partition-table algorithm is the authoritative implementation-facing account.

\end{subsection}

\begin{subsection}{Places where the documents do not determine one edit}

There are several places where the newest file does not contain a settled
final answer, even though it is clearly the newest file.  These are not source
chronology ambiguities; they are mathematical or editorial decisions still
visible in the colored comments.

\begin{enumerate}
\item The early theorem preview still risks conflating state-address capacity
with path-address capacity.  The targeted note
\path{docs/design/logHRL_state_vs_path_address_capacity_note.tex}
argues that the fix is to distinguish state/cell address factors
$b_i^{\mathcal S}$ from path/refinement address factors $b_i^\Omega$.  The
later path-volume theorem is not thereby killed, but its $b_i$ should be read
as path-address branching, not merely discovered-state branching.

\item The reward direct-image language is not fully settled.  The current
comments correctly resist treating ``direct image'' as synonymous with an
average.  Sum, expectation, weighted sum, max, soft max, RMS, and other
aggregation operators may represent different quotient-task semantics.  A
final edit should state this as an aggregation-design surface rather than a
single forced formula.

\item The Markov/history issue is open at the prose level.  The current draft
sometimes gestures toward policies on histories, but the package and theorem
are still primarily Markov/action-local after suitable state augmentation.
The final paper should decide whether to restrict the displayed model to the
Markov setting, or to introduce history-dependent policies only as future
work.

\item The stutter and loop-removal convention needs one explicit statement.
The current algorithm says that internal loops are discarded after coalescence,
but the comments ask whether these loops are immaterial or whether a stutter
convention should be declared.  This is an implementation-facing mathematical
choice and should not be silently hidden.

\item The algorithm input/schema assumptions need tightening.  The current
comments ask whether the function needs the current state $s_t$, the whole
state set $\mathcal S_t^0$, or only the graph and the contraction schema.  The
newer runtime picture suggests that the graph plus its ordered base-edge
partition schema are the real construction inputs, while $s_t$ is needed only
for active-agent readout.

\item The runtime coalescence bound needs a standard data-structure reference
and cleaner notation.  The draft currently uses union/coalescence language in
a way that is directionally right, but PM comments ask whether the $U_i$
quantities and logarithmic representative-change bounds are stated in the
right form.  This should be reconciled with standard union-find or
partition-maintenance references before finalizing the theorem-level runtime
claim.

\item The U-Net/MgNet citation thread is unresolved.  The comments ask whether
the relevant citation is MgNet 2019, the later 2026 multigrid-efficiency paper,
or both.  This is a citation and conceptual-positioning issue rather than a
source-version issue.

\item The log/tropical and adic notes contain a genuinely later mathematical
thread, but they are not yet integrated into the paper's main theorem.  Folding
them into this paper would change the paper's scope.  The safest present
state is to preserve them as companion notes unless PO decides the main paper
should become a broader log/tropical geometry essay.
\end{enumerate}

\end{subsection}

\begin{subsection}{Recommended next editorial pass}

The next pass should not try to merge by copying every older paragraph into
the current draft.  The better sequence is:

\begin{enumerate}
\item Keep this file as the working merged spine.
\item Resolve the state-address/path-address notation first, because it
controls the main theorem.
\item Resolve reward aggregation and Markov/history scope next, because those
control the assumptions.
\item Then clean the algorithm input, loop/stutter convention, and runtime
bound.
\item Only after the main theorem is stable, decide whether the log/tropical
and adic notes belong as appendices or as a separate companion document.
\end{enumerate}
