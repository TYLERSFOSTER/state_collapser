...
\ \ \ \ \ All figures were created by Tyler Foster in \texttt{draw.io} and \texttt{LaTeX}.
\end{minipage}
\end{center}
...
For residual correction, it is {\em multigrid}-like, in the sense of \cite{AhamedZakariaeiHaberEliasofMGE2026}. MgNet \textcolor{red}{Do you mean this \cite{he2019mgnet} article?}makes this correspondence especially explicit: restriction maps pass information to a coarser grid, smoothing operations remove local high-frequency error, and prolongation maps lift coarse corrections back to the fine scale. Read through this lens, a residual CNN is not merely a stack of feature extractors; it is also an iterative correction scheme in which coarse representations control the search for fine-scale residual structure.
...
First, you should be thinking of $\bold{Path}(H)$ as ``something {\em waaaaay} worse/more complicated than the fundamental group $\pi_{1}(H,\ast)$ (for some choice of node $\ast$ in $H$). The fundamental group $\pi_{1}(H,\ast)$ is hard to understand in general. The $\bold{Path}(H)$ space is sort of unknowable. The {\em path space} is the set of all paths {\em of all lengths} in $H$. The {\em length-0} paths are the same things as nodes. The {\em length-1} paths are just edges. All paths of length $\ge 2$ contain more than one edge. 

The relevance to RL is that a ``point'' $\gamma\in\bold{Path}(H)$ amounts to a history in $H$, i.e., a sequence of consecutive nodes (states) connected by edges (primitive actions). So the reason for even thinking about $\bold{Path}(H)$ in the context of RL is that $\bold{Path}(H)$ is the moduli space of things that RL training needs to search over and optmize.

It is {\em very much} like the path formulation of QM, where reality is somehow optimizing over all paths to minimize this physical quantity $S$ (``action'' in a different sense than we're using here).

{\em The real problem} we run into is that because RL is trying to optimize over this moduli space of histories $\bold{Path}(H)$, we have to actually do some kind of geometry on $\bold{Path}(H)$. We can't just treat it like a set. The path formulation of QM runs into this exact problem: this ``problem of summing over all Feynman diagrams'' that you hear about sometimes is really an attempt to take an integral roughly of the form
$$
\underset{\text{Path}(TX)}{\int}\!\!\!\!\!\!L(\gamma)\,d\gamma,
$$
where $\text{Path}(TX)$ denotes all histories through phase space, and where ``$L(\gamma)$'' is some physical quantity you can compute, given a particular history $\gamma$. Mathematicians look at that integral and think ``Are you joking?''

Kontsevich's big achievement around ``{\em stable maps} and ``{\em virtual fundamental classes} was to show that using algebro-geometric tools, you can build something {\em like} $\text{Path}(TX)$, but much more controlled, namely a moduli space of stable curves, where integration makes sense, in the form of an intersection theory. I am conflating some thigns here, and being loose with ideas, but it's the correct general picture.
...
\end{enumerate}
This project exist \textcolor{red}{exists} in the second and third layers. The quotient tower we build is a structured computational representation through which the agent can move more efficiently.
\end{ermark}
...
\begin{definition}
\normalfont
A finite directed {\em state/action-graph} $H$ \textcolor{red}{perhaps emphasize all four words: finite, directed state/action-graph $H$}is a finite directed graph
...
{\bf Region of 3-dimensional space as la \textcolor{red}{remove l?} grid lattice:}
\normalfont
...
\;\le\;\delta
$$
\textcolor{red}{do you mean $\delta_H$?}
...
\;\xrightarrow{\ \ \ \ \ \ \ }\;H
$$
We will often describe a history $\gamma$ in $H$ by sequence of states and actions in its image, \textcolor{red}{comma needs to be removed}by writing
...
\normalfont
The path space of $H$ is the discrete analogue of the usual topological path space $\Path(X):=C^0([0,1],X)$ associated to a topologicla \textcolor{red}{topological} space $X$
...
$$
a\textcolor{red}{\alpha}\;\mapsto\;r(\alpha)
$$
\end{remark}
...
\normalfont
When every one of these distributions $P_{\!\alpha}$ is a Dirac delta supported at some point $r(\alpha)\in\mathbb{R}$, we can understand the action-local stochastic kernel as a \textcolor{red}{deterministic?}{\em action-local reward function}
$$
...
\item
There exists some sequence $w_{\bullet}=(w_{0}, \;w_{1},\;w_{2},\;\cdots \textcolor{red}{,w_{k-1}})$ of {\em decay weights} $w_{i}\in\mathbb{R}_{\ge0}$, such that for any history $\gamma=(s_{0}\xrightarrow{\ \ \alpha_{1}\ }s_{1}\xrightarrow{\ \ \alpha_{2}\ }\cdots\xrightarrow{\ \alpha_{k-1}\ }s_{k-1}\xrightarrow{\ \alpha_{k}\ }s_{k})$, of any finite length $k$, its associated reward can be computed as the convolution product $r(\gamma)=w_{\bullet}\ast r(\alpha_{\bullet})$, i.e.,
$$
    P_{\gamma}\;\;=\;\;\sum_{i=0}^{k-1} w_{i}\,P_{\alpha_{k-i}}
$$
\end{enumerate}
\end{definition}
\textcolor{red}{Very interesting! Is there a nice way to compute the weights? Like some sort of Parseval's identity?}
\begin{remark}
\normalfont
...
\end{ermark}
\textcolor{red}{Sorry, what does the $\Delta$ stand for? I have never taken a course on Markov Processes, so I don't know if that's standard terminology. Sorry!. Scratch that. It's a distribution}
\begin{ermark}
{\bf Policies as models-plus-weights on a concrete mahcine\textcolor{red}{machine}}
\normalfont
If we want to take a materialist's perspective, our policy is {\em really} just some familiar old ML model, a DNN, or a transformer, or a U-NET, or whatever.  And we train this model in exactly the same way we always to\textcolor{red}{do}: we repeatedly evaluate the model’s performance on some set of tasks, and we adjust the model, in response, via gradient backpropagation.

It’s really the same old thing. 
\end{ermark}

The only part of this whole situation that’s {\em really} different from vanilla ML is that for RL, our labelled training data is ``stored'' in a {\em virtual rollout database}, and that our strategy for taken \textcolor{red}{taking?} entries from this database is different than for vanilla ML.
...
\normalfont
One of the central insights from \cite{MalikThesis} is that in lots of graph-structured problems, we can can improve search times by putting the graph into a tower of randomly chosen contractions that functions as a \textcolor{red}{slash em}{|em graph quotient HNSW} query system.
...
$$
Likewise, we define the associated {\em viewed subgraph at time $t$}, denoted $G^{0}_{t}$, to be the subgraph of $H$ formed by the union of the visited subgraph $H^{0}_{t}$ at times \textcolor{red}{time} step $t$ and all outgoing $1$-hop neighborhoods $N^{\text{out}\!}_{1}(s)$ in $H$ at nodes in $H^{0}_{t}$ at time step $t$:
$$
G^{0}_{t}
\;:=\;
...
\normalfont
Given a history as in \eqref{eq: gamma history}, we decorate each node $s$ of $G^{0}_{t}$, at each time step $t$, with its set of outgoing edges \textcolor{red}{from?} $\mathcal{A}_{H}(s)$ {\em inside} our ambient, hidden state/action-graph $H$.
\end{definition}
...
We now contract all edges in $G^{0}_{t}$. However, we do to in a tiered manner, so that we contract edgers \textcolor{red}{edges} in stages, eventually exhausting all edges. 

We want to allow for both of the possibilities in Examples \ref{ex: actions match projections} and \ref{ex: point cloud}. In the case of Example \ref{ex: actions match projections}, the most obvious contraction schema is: (i) contract all $x$-axis actions, (ii) contract all $y$-axis actions, (iii) contract all $z$-axis actions. On a machine, this would mean labelling actions (\textcolor{red}{``$x$'' instead of typewriter text?}\texttt{`x`}, ``$y$,'' or ``$z$'') and contracting according to label. In the case of Example \ref{ex: point cloud}, the randomness of the edges in $3$-space, and the randomness of out-degree from node to node, makes it difficult to contract them in any systematic way. A more natural strategy in this case is to sample $1/3$ of the outgoing, not yet \textcolor{red}{un?}sampled edges at each node in the original graph, and collapse them all to get next tier.

Although these two contraction schemas are very different, they have the same procedural outline, which we formalize as a ``contraction schema''\textcolor{red}{.}
...
We implement the quotient-tower in RAM by maintaining a nested system of labeled partitions of the base state set and base edge set under successive coarsening operations. This places the method near the classical partition-based data-structural viewpoint in algorithms \cite{PaigeTarjan1987} and \cite{HabibPaulViennot1999}. That said, the construction we now describe is not a \textcolor{red}{partition}{\em partion refinement algorithm} in the usual sense. Because the operative updates here encode graph edge contractions, they take the form of {\em coarsenings} of coupled state and action \textcolor{red}{state/action? just for consistency} coset partitions. The tower is best understood as a coupled\textcolor{red}{,} nested partition system adapted to contraction-driven quotient formation.
...
Inductively, suppose the tier-$(i-1)$ state and action partition tables have already been created. The $i$th tier is produced by opening fresh tier-$i$ tables initialized from tier $i-1$\textcolor{red}{tier-$(i-1)$?}, then processing every arrow in the contraction block $\Sigma^i$. For an arrow $a\xrightarrow{\ e\ }b$, we look up the tier-$(i-1)$ cells containing $a$ and $b$. If they are distinct, we create a new tier-$i$ state-cell representing their union and a new tier-$i$ outgoing action-cell collection representing the union of their outgoing data, after discarding actions that have become loops inside the merged cell \textcolor{red}{What goes wrong if we don't discard the loops? Are they completely immaterial? Should we say something about a chosen stutter convention here from Definition \ref{def:tier-i-path-address}}. The affected base nodes receive tier-$i$ pointers to the new cell. Repeating this through the block yields $G_t^i$.

The inductive step from tier $i-1$ \textcolor{red}{tier-$(i-1)$?} to tier $i$ is driven by the $i$th block $\Sigma^i$ of the base-edge contraction schema. For each edge $a\xrightarrow{\ e\ }b$ in this block, let $C_a$ and $C_b$ be the current state-cells containing its endpoints. If $C_a\neq C_b$, we create a new tier-$i$ state-cell $C_{ab}$ by coalescing $C_a$ and $C_b$, and we create the corresponding tier-$i$ action-cell data by coalescing their outgoing action-cell collections. Pointers from all nodes in $C_a\cup C_b$ are then initiated to $C_{ab}$, and $C_{ab}$ is pointed to its merged outgoing-action collection. Repeating this for all arrows in $\Sigma^i$ gives the next partition layer.
...
\end{equation}
...There's also a natural argument that the expect \textcolor{red}{expected} nature of the sparsity in state/action graphs gives us the following expected value of $m_{t}$ is
$$
m_{t}
\;\sim\;
n_{t}\,d
...
$
Thus \textcolor{red}{`Note that' instead of `thus'? Because I don't know it follows immediately from the sum-of-cardinality-of-cosets-equals-cardinality-of-set. At least it is not immediately obvious to me.} every base-tier edge is processed exactly once by the contraction schedule.
...
First, the algorithm copies the previous partition tables forward and later re-initiates the node pointers for all states. The explicit final loop\textcolor{red}{you can replace this with the actual line number \ref{line:last-loop-algo-init}}
\[
...
\begin{itemize}
    ...
    \]
\end{itemize}
\textcolor{red}{Would one coalescences not be treated as a single step in almost any language? Then $U^{\mathcal{S}}_i = \left\vert\Pi^{\mathcal{S}}_i\right\vert$ and $U^{\mathcal{A}}_i = \left\vert\Pi^{\mathcal{A}}_i\right\vert$, and the $U_i$'s would depend on $i$}.
...
\textcolor{red}{I would maybe use a breakable algorithm}
\begin{ermark}
...
$$
An entropy-based variant replaces support size by $\exp(H(\mu_{\theta,k}))$.\textcolor{red}{What is $H$? I thought it referred to a graph. I don't know what the exponential means here with that distribution.}
\end{definition}
...
\sum_{k\ge0}d^k
$
is useful as a slogan for path-space explosion, but it is not a finite complexity invariant unless it is truncated or discounted. All statements below use a finite horizon $K$, or else \textcolor{red}{remove `else'?} an explicitly discounted variant.
\end{warning}
